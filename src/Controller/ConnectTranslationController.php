...
/**
 * @file
 * Contains \Drupal\connect_translation\Controller\ContentTranslationController.
 */

namespace Drupal\connect_translation\Controller;
 ...
 */
class ConnectTranslationController extends ControllerBase {
   ...
   */
    public function prepareTranslation(ContentEntityInterface $entity, LanguageInterface $source, $target, $target_node) {
    /* @var \Drupal\Core\Entity\ContentEntityInterface $source_translation */
    $source_translation = $entity->getTranslation($source->getId());

    $target_translation = $entity->addTranslation($target->getId(), $target_node->toArray());
  ...
  public function overview(RouteMatchInterface $route_match, $entity_type_id = NULL) {
    ...
    if ($this->languageManager()->isMultilingual()) {
      ...
      foreach ($languages as $language) {
        ...
        $add_url = new Url(
          'connect_translation.translation_connect_' . $entity_type_id,
          array(
            'source' => $original,
   ...
   */
  public function add(LanguageInterface $source, LanguageInterface $target, $target_node_id, RouteMatchInterface $route_match, $entity_type_id = NULL) {
    $entity = $route_match->getParameter($entity_type_id);


    $target_node = \Drupal::entityManager()->getStorage('node')->load($target_node_id); 

    // @todo Exploit the upcoming hook_entity_prepare() when available.
    // See https://www.drupal.org/node/1810394.
    $this->prepareTranslation($entity, $source, $target, $target_node);
